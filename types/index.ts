...
// === 완전학습 (Mastery Learning) 관련 타입 정의 ===
export interface MasteryTopic {            // 오늘 배운 것 = 채점 기준
  id: string; 
  date: string;
  subject: string; 
  topic: string;                  // 단원 / 주제
  keyPoints: { id: string; text: string }[];  // 인출해야 할 핵심 포인트
  classId?: string;
}

export type RecallMethod = 'blank_write' | 'verbal' | 'peer_explain'; // 백지/구술/또래설명

export interface MasteryCheck {            // 학생별 백지 테스트 결과
  id: string; 
  studentId: string; 
  topicId: string; 
  date: string;
  method: RecallMethod;
  results: { pointId: string; recalled: boolean }[];
  retrievalScore: number;           // 0~100 = 인출 포인트수 / 전체
  studentDump?: string;             // (선택) 학생이 타이핑한 인출 원문
  note?: string;
  peerTo?: string;                  // 누구에게 설명했는지(학생명/ID)
}

export interface Gap {                     // 구멍 (결손)
  id: string; 
  studentId: string;
  subject: string; 
  topic: string;
